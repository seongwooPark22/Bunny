...
int main(int argc, char* argv[])
{
	...
	window = glfwCreateWindow(WIDTH, HEIGHT, "Q1-OpenGL Bunny", NULL, NULL);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < indices.size(); i++) {
			glm::vec3 v = vertex_position_buffer[indices[i]];
			glm::vec3 v_n = vertex_normal_buffer[indices[i]];
		...
		char string[1024] = { 0 };
		sprintf(string, "Q1-OpenGL Bunny: %0.2f FPS", fps);
		avg_fps += fps;
